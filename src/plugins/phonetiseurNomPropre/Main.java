...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args[0].equals("-a")) {
            ...
            // On charge JHTK
            String[] tDicos = new String[args.length - 4];
            ...
            phonetiseur.lancerApprentissage(bdlex, args[args.length - 2], args[args.length - 1]);
            ...
            TreeMap<String, float[]> treeProbasLangages = phonetiseur.phonetiseur.getTreeMapProbasLanguesLog(args[3]);

            BufferedReader r = FileUtils.openFileUTF(args[2]);
            PrintWriter w = FileUtils.writeFileUTF(args[4]);
            ...
            while ((sMotAPhonetiser = r.readLine()) != null) {
                ...
                if (!DicoGraphemesPhonemes.motNonRejete(t[0])) {
                    ...
                    try {
                        w.println(t[0] + ";" + ((posTag == null) ? "" : posTag) + ";" + phonetiseur.phonetiser(t[0], posTag, treeProbasLangages.get(t[0].replace('\'', Configuration.CHAR_DE_REMPLACEMENT_APPOSTROPHE).toLowerCase())));
                    } catch (Exception ex) {
    ...
    public static void afficherAideEtFin() {
        System.out.println("\n\n" +
                ...
                "    \"Main\" -a repDonnees fichierDico_1 ... fichierDico_n fichierMotsAApprendre fichiersProbasLangages\n" +
                "Pour executer une phonetisation :\n" +
                "    \"Main\" -p repDonnees fichier_mots_posTags fichiersProbasLangages fichierResultats\n\n" +
                "Avec :\n" +
                ...
                "  - fichiersProbasLangages : fichier (au format UTF8) contenant (par ligne) : le mot suivit des probas (en log). Le s�parateur est l'espace (un et un seul). Attention : pas d'espace a la fin de la ligne\n");
        System.exit(0);
    }
