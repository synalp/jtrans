...
import fr.loria.synalp.jtrans.utils.DET;
...
public class SpeechRecoAccuracy {
	 ...
	 */
	public static List<RecoWord2> normalizeMots(List<RecoWord2> mots, boolean decoup) {
		for (RecoWord2 mot : mots) {
		...
		if (decoup) {
			...
			ArrayList<RecoWord2> rec2 = new ArrayList<RecoWord2>();
			for (RecoWord2 w : mots) {
				if (w.word.length()==0) continue;
				String[] ss = w.word.split(" ");
				if (ss.length>1) {
					for (String ww : ss) {
						RecoWord2 rw = new RecoWord2(ww, w.conf);
						rec2.add(rw);
					}
				} else rec2.add(w);
			}
	...
	public static void confidenceDET(DET det, String ctmfile, String labfile) {
		try {
			...
			List<RecoWord2> rec = new ArrayList<RecoWord2>();
			for (;;) {
				String s= f.readLine();
				if (s==null) break;
				String[] ss = s.split(" ");
				RecoWord2 w = new RecoWord2(ss[ss.length-2], Double.parseDouble(ss[ss.length-1]));
				rec.add(w);
			}
			f.close();
