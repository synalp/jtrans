...
package jtrans.speechreco.acousticModels.HMM.stateModels;
...
import jtrans.speechreco.acousticModels.HMM.Lab;
import jtrans.speechreco.acousticModels.HMM.LogMath;

public class GMMDiag extends HMMState {
	public int nT = 0;
	public LogMath logMath = null;

	private int ncoefs = 0;
	private int ngauss = 0;
	protected float[] weights = null;
	protected float[][] means = null;
	protected float[][] covar = null;
	private float[] logPreComputedGaussianFactor = null;
	protected float[] loglikes = null;
	...
	public boolean isEmitting() {return true;}
	
	public GMMDiag clone() {
		GMMDiag gmm = new GMMDiag(getNgauss(),getNcoefs());
		for (int i=0;i<getNgauss();i++) {
			gmm.setWeight(i, getWeight(i));
			System.arraycopy(means[i], 0, gmm.means[i], 0, getNcoefs());
			System.arraycopy(covar[i], 0, gmm.covar[i], 0, getNcoefs());
		}
		System.arraycopy(logPreComputedGaussianFactor, 0, gmm.logPreComputedGaussianFactor, 0, getNgauss());
		gmm.lab = new Lab(lab);
		return gmm;
	}
    ...
    /**
     * je divise le calcul des loglikes pour pouvoir faire de la marginalisation !
     * 
     * @param data
     */
	public void computeLogLikes(float[] data) {
		...
		for (int gidx = 0; gidx < ngauss; gidx++) {
			...
			for (int i = 0; i < data.length; i++) {
				float logDiff = (float) data[i] - means[gidx][i];
...
*/
	...
	/**
	 * get a loglike vector (1 per Gauss)
	 * @return
	 */
	public float[] getLogLikes() {
		return loglikes;
	}
