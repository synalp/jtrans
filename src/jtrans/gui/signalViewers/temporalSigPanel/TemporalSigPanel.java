...
import jtrans.elements.SpeakerTurn;
...
public class TemporalSigPanel extends JComponent {
	...
	//----------------- Constructor ----------------------------
	//----------------------------------------------------------
	public TemporalSigPanel(JTransGUI fen){
		super();
...
//		buffer.print=true;
		...
		//-----------------------------------------------------------
		ElementList elementList = aligneur.project.elts;
		int posiLine = 0;
		...
		boucleMot:while (i < elementList.size()){
			...
			if(element instanceof SpeakerTurn){
				...
				indiceLocuteur = ((SpeakerTurn)element).getLocuteurID();
			}
			else if (element instanceof Word){
				Word elm = (Word)element;
				motidx++;
				if (showPhones) {
				} else {
					long lastSample = TimeConverter.frame2sample(aligneur.project.words.getSegmentEndFrame(elm.posInAlign));
					Word elementAvecDuree = (Word)element;
					...
					if(posiLine > 0){
						...
						if(elementAvecDuree instanceof Word){
							Word elementMot = (Word)element;
		...
		if (showPhones) {
			...
			int frdeb = TimeConverter.sample2frame(offsetStart);
...
System.out.println("debugsegtoprint "+segidx);
			if (segidx>=0) {
				...
				long endSampleOfSeg = TimeConverter.frame2sample(endFrOfSeg);
				...
				if(posiLine > 0){
					...
					for (int k=segidx+1;k<aligneur.project.phons.getNbSegments();k++) {
						...
						endSampleOfSeg = TimeConverter.frame2sample(endFrOfSeg);
						posiLine = (int) ((endSampleOfSeg-offsetStart)/hZoom);
						if(posiLine > 0){
...
*/	
	...
	public boolean setProgressBar(long readedShort){
		...
		aligneur.toolbar.setPos(TimeConverter.getTimeHMinSFromSeconds(TimeConverter.sample2second(readedShort)));
	...
	public boolean setProgressBar(long sample, float posPB){
		aligneur.toolbar.setPos(TimeConverter.getTimeHMinSFromSeconds(TimeConverter.sample2second(sample)));
		int widthFoisHZoom = getWidth()*hZoom;
			...
			if(selectedLineIndice >= 0){
				...
				if(elementList.size() > 0){
					...
					if(selectedLineIndice > 0){
						...
						Word motPrecedent = elementList.getElementAvecDureePrecedent(selectedLineIndice);
					...
					if(selectedLineIndice < elementList.size()){
						//pour empecher de depasser la barre de fin du mot suivant
						Word prochainMot = elementList.getElementAvecDureeSuivant(selectedLineIndice);
...
//					aligneur.project.words.setEndSample(((Element_Mot)(listeElement.get(selectedLineIndice))).getIndexInAlignement(), positionLineInSample);
				...
				/*
					...
					int deltafr = TimeConverter.sample2frame(valeurDeplacement);
...
//					aligneur.project.words.setAlignForWord(selectedWordIndice, minfr-1, newfr);
		...
		private int getIndiceBarrePointee(MouseEvent e){
			...
			if(   (y < lineBase) && ( y > lineMax) ) {
				...
				List<Word> listeMot = elementList.getMots();
				...
				while ((i < listeMotSize) 
					...
					if( Math.abs(x - posiLine) < ecartAutorise ){
						return elementList.indexOf(listeMot.get(i));
					}//if 
					i++;
				}//while
		...
		private int getIndiceMotPointe(MouseEvent e){
			...
			if( (y < lineBase ) && ( y > lineMax) ) {
				...
				if (segidx>=0) {
					int motidx=0;
					for (Word m : aligneur.project.elts.getMots()) {
...
//					cursample-=5000; // decalage visible... ?!
	...
	public void moveAtFrame(int fr) {
		long sa = TimeConverter.frame2sample(fr);
		setProgressBar(sa);
	}
