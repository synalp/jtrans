...
import jtrans.speechreco.acousticModels.HMM.LogMath;
import java.io.*;
...
public class Phonetiseur {
    ...
    public void chargerMatriceProbaEtLexique(String fichierMatrices) throws IOException {
        BufferedReader f = FileUtils.openFileUTF(fichierMatrices);
        String[] t;
     ...
     */
    public void genererFichiersVecteursARFF(String repertoire, String fichierProbasLangues) throws IOException {
        ...
        // Etape 1 :
        // On recupere les probas liees aux langues
        TreeMap<String, float[]> treeProbasLangues = getTreeMapProbasLanguesLog(fichierProbasLangues);

        // Etape 2 :
        // On genere les fichiers ARFF
        ...
        // Les entetes
        String[] entetes = getARFF_Entete_Methode_4Classifieurs(lexique.getStringGraphemes(","), lexique.getStringPhonemesSimples(","), lexique.getStringPosTag(","), Configuration.GRAPHEME_DEBUT_DE_MOT_VECTEUR, Configuration.GRAPHEME_FIN_DE_MOT_VECTEUR, treeProbasLangues.firstEntry().getValue().length);
        ...
        for (Entry<String, LinkedList<SuitePhonemes>> entry : dico.getDico().entrySet()) {
            for (SuitePhonemes suitePhonemes : entry.getValue()) {
                ...
                tContenuARFF = agp.getVecteur_Methode_4Classifieurs(Configuration.GRAPHEME_DEBUT_DE_MOT_VECTEUR, Configuration.GRAPHEME_FIN_DE_MOT_VECTEUR, suitePhonemes.getPosTag(), stringGraphemeVide, treeProbasLangues.get(entry.getKey()));
     ...
     * @return
     */
    public static String[] getARFF_Entete_Methode_4Classifieurs(String graphemes, String phonemes, String postag, String graphemeDebut, String graphemeFin, int nbLangues) {
        ...
        for (int i = 0; i < nbLangues; i++) {
            t[0] += "@attribute langue" + (i + 1) + " numeric\n";
        }
        t[0] += "@attribute typePhoneme {simple,double}\n" + "\n@data\n";
        ...
        t[1] = "@relation SimplesPhonemes\n" +
                ...
                "@attribute posTag {" + postag + "}\n";
        for (int i = 0; i < nbLangues; i++) {
            t[1] += "@attribute langue" + (i + 1) + " numeric\n";
        }
        t[1] += "@attribute phoneme {" + phonemes + "}\n" + "\n@data\n";
        ...
        t[2] = "@relation 1erDoublesPhonemes\n" +
                ...
                "@attribute posTag {" + postag + "}\n";
        for (int i = 0; i < nbLangues; i++) {
            t[2] += "@attribute langue" + (i + 1) + " numeric\n";
        }
        t[2] += "@attribute phoneme {" + phonemes + "}\n" + "\n@data\n";
        ...
        t[3] = "@relation 2emeDoublesPhonemes\n" +
                ...
                "@attribute posTag {" + postag + "}\n";
        for (int i = 0; i < nbLangues; i++) {
            t[3] += "@attribute langue" + (i + 1) + " numeric\n";
        }
        t[3] += "@attribute phoneme {" + phonemes + "}\n" + "\n@data\n";
     ...
     */
    public String[] phonetiser(String[] tGrapheme, String posTag, float[] tProbasLangage) throws Exception {
        return classifieurs.phonetiser(tGrapheme, posTag, tProbasLangage).getPhonemes();
    }
     ...
     */
    public double testerAvecPosTag(PronunciationsLexicon pronuncitationsLexicon, String cheminFichierMotsTests, String cheminFichierMauvaisAlignements, String fichierProbasLangues) throws Exception {
        TreeMap<String, float[]> treeProbasLangues = getTreeMapProbasLanguesLog(fichierProbasLangues);
        ...
        for (Entry<String, LinkedList<SuitePhonemes>> entry : dicoTest.getDico().entrySet()) {
            ...
            for (int i = 0; i < alPostag.size(); i++) {
                erreur = true;
                agp = classifieurs.phonetiser(tGraphemes, alPostag.get(i), treeProbasLangues.get(entry.getKey()));
     ...
     * @return le pourcentage de reussite
     */
    public double testerSansPosTag(PronunciationsLexicon pronuncitationsLexicon, String cheminFichierMotsTests, String cheminFichierMauvaisAlignements, String fichierProbasLangues) throws Exception {
        TreeMap<String, float[]> treeProbasLangues = getTreeMapProbasLanguesLog(fichierProbasLangues);
        LexiqueGraphemesPhonemesPostag l = new LexiqueGraphemesPhonemesPostag();
        DicoGraphemesPhonemes dicoTest = new DicoGraphemesPhonemes(pronuncitationsLexicon, cheminFichierMotsTests, l);
        String[] tGraphemes, tPhonemesResultat;
        ...
        for (Entry<String, LinkedList<SuitePhonemes>> entry : dicoTest.getDico().entrySet()) {
            ...
            erreur = true;
            agp = classifieurs.phonetiser(tGraphemes, null, treeProbasLangues.get(entry.getKey()));
            tPhonemesResultat = agp.getPhonemes();
            nbTotalMot++;
    ...
    /******************************** Utils ********************************/
    /***********************************************************************/
    public TreeMap<String, float[]> getTreeMapProbasLanguesLog(String fichierProbasLangues) throws IOException {
        TreeMap<String, float[]> treeProbasLangues = new TreeMap<String, float[]>();
        BufferedReader r = FileUtils.openFileUTF(fichierProbasLangues);
        String ligne;
        while ((ligne = r.readLine()) != null) {
            String[] tString = ligne.split(" ");
            LogMath lm = new LogMath();

            float[] tProbasLog = new float[tString.length - 1];

            for (int i = 0; i < tProbasLog.length; i++) {
                tProbasLog[i] = Float.parseFloat(tString[i + 1]);
            }
            float somme = tProbasLog[0];
            for (int i = 1; i < tProbasLog.length; i++) {
                somme = lm.addAsLinear(somme, tProbasLog[i]);
            }
            for (int i = 0; i < tProbasLog.length; i++) {
                tProbasLog[i] -= somme;
            }
            ...
            String mot = tString[0].replace('\'', Configuration.CHAR_DE_REMPLACEMENT_APPOSTROPHE).toLowerCase();

            treeProbasLangues.put(mot, tProbasLog);
        }
        return treeProbasLangues;
    }
}
